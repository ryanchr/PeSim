...
bpred_create(enum bpred_class bprdclass,	/* type of predictor to create */
	     unsigned int bimod_size,	/* bimod table size */
	     unsigned int l1size,	/* 2lev l1 table size */
	     unsigned int l2size,	/* 2lev l2 table size */
	     unsigned int meta_size,	/* meta table size */
	     unsigned int shift_width,	/* history register width */
	     unsigned int bpr_xor,  	/* history xor address flag */
	     ...
	     unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  case BPredComb:
    ...
    /* 2-level component */
    pred->dirpred.twolev = 
      bpred_dir_create(BPred2Level, l1size, l2size, shift_width, bpr_xor);
  ...
  case BPred2Level:
    pred->dirpred.twolev = 
      bpred_dir_create(bprdclass, l1size, l2size, shift_width, bpr_xor);

    break;

  case BPred2bit:
    pred->dirpred.bimod = 
      bpred_dir_create(bprdclass, bimod_size, 0, 0, 0);
...
bpred_dir_create (
  enum bpred_class bprdclass,	/* type of predictor to create */
  ...
  unsigned int bpr_xor)	    	/* history xor address flag */
{
  ...
  case BPred2Level:
	...
	fatal("shift register width, `%d', must be non-zero and positive",
      ...
      pred_dir->config.two.bpr_xor = bpr_xor;
  ...
  FILE *stream)			/* output stream */
{
  ...
  case BPred2Level:
    fprintf(stream,
      ...
      pred_dir->config.two.bpr_xor ? "" : "no", pred_dir->config.two.l2size);
    break;
		 ...
		 md_addr_t baddr)		/* branch address */
{
  ...
  switch (pred_dir->bprdclass) {
	...
	int l1index, l2index;
        ...
        if (pred_dir->config.two.bpr_xor)
	  {
#if 1
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  if (pred->bprdclass == BPredNotTaken || pred->bprdclass == BPredTaken)
    return;
  ...
  if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND) &&
      (pred->bprdclass == BPred2Level || pred->bprdclass == BPredComb))
    {
