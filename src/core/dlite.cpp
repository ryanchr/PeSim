	      ...
	      int nbytes)			/* size of access */
{
  ...
  /* else, no error, access memory */
	fatal("dlite_mem_obj() >> mem_access nodefined\n");
...
bp_class_str(int dlt_class)				/* breakpoint class mask */
{
  if (dlt_class == (ACCESS_READ|ACCESS_WRITE|ACCESS_EXEC))
    return "read|write|exec";
  else if (dlt_class == (ACCESS_READ|ACCESS_WRITE))
    return "read|write";
  else if (dlt_class == (ACCESS_WRITE|ACCESS_EXEC))
    return "write|exec";
  else if (dlt_class == (ACCESS_READ|ACCESS_EXEC))
    return "read|exec";
  else if (dlt_class == ACCESS_READ)
    return "read";
  else if (dlt_class == ACCESS_WRITE)
    return "write";
  else if (dlt_class == ACCESS_EXEC)
    return "exec";
  else
...
static char *					/* err str, NULL for no err */
set_break(int dlt_class,				/* break class, use ACCESS_* */
	  struct range_range_t *range)		/* range breakpoint */
{
  struct dlite_break_t *bp;

  /* add breakpoint to break list */
  bp = (dlite_break_t*)malloc(sizeof(struct dlite_break_t));
  if (!bp)
    fatal("out of virtual memory");

  bp->id = break_id++;
  bp->range = *range;
  bp->dlt_class = dlt_class;
