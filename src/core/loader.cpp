...
static Uint32 start = 0x56900;
static Uint32 end = 0x9ffff;
...
ld_load_prog(char *fname,		/* program to load */
	     ...
	     struct mem_t *mem,		/* memory space to load prog into , not used now*/
	     int zero_bss_segs,		/* zero uninit data segment? */
             Bus_slow_mem *mem_slow)/* memory space to load prog into */
{
 debugging = 0;
  int i;
  word_t temp;
  ...
  if (eio_valid(fname))
    ...
    {
    printf("MD_CROSS_ENDIAN defined \n");
	  ...
	  && (shdr.sh_type == SHT_PROGBITS || shdr.sh_type == SHT_NOBITS))
	{
	  ...
	  if ((shdr.sh_flags & SHF_ALLOC) != 0 && shdr.sh_type != SHT_NOBITS)
		...
		fatal("could not read all the contents of section");
	      ...
	      printf("shdr.sh_addr = 0x%0x\n", shdr.sh_addr);
	    mem_slow->MemBcopy(CmdWrite, shdr.sh_addr, buffer, shdr.sh_size);
	    }
  ...
  /* write [argc] to stack */
  temp = argc;
  mem_slow->MemAccess(CmdWrite, sp, &temp, sizeof(word_t));
  ...
  for (i=0; i<argc; i++)
    {
      ...
      temp = sp;
      mem_slow->MemAccess(CmdWrite, argv_addr + i*sizeof(md_addr_t),
		 &temp, sizeof(md_addr_t));
      /* and the data */
      mem_slow->MemStrcpy(CmdWrite, sp, argv[i]);
      sp += strlen(argv[i])+1;
    }
  ...
  for (i = 0; envp[i]; i++)
    {
      ...
      temp = sp;
      mem_slow->MemAccess(CmdWrite, envp_addr + i*sizeof(md_addr_t),
		 &temp, sizeof(md_addr_t));
      /* and the data */
      mem_slow->MemStrcpy(CmdWrite, sp, envp[i]);
      sp += strlen(envp[i]) + 1;
    }
  ...
  if (sp > ld_stack_base)
    {
	 ...
	 command line arguments */
      debug("environment overflow, increase MD_MAX_ENVIRON in arm.h");
    }
