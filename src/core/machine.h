...
/* Storage for banked registers (note, most modes only need 2 banked registers
 * but may need space to save data if switching into or out of FIQ */
typedef struct {
  word_t FIQ_REGS[8];
  word_t FIQ_SPSR;
...
/* Coprocessor 15 registers (pointers to!!) */
/* NOTE: These are POINTERS ONLY. Simulator initialization MUST map
         these to actual registers (wherever they may actually be)
	 or create space for them.
*/
typedef word_t *md_cp15_t[16];
...
#define UND_MODE 0x1a

#define _PSR_IRQ(PSR)		(((PSR) >> 7) & 1)
#define _SET_PSR_IRQ(PSR, VAL) ((PSR) = (((PSR) & ~(1 << 7)) | ((VAL & 1) << 7)))
#define _PSR_FIQ(PSR)		(((PSR) >> 6) & 1)
#define _SET_PSR_FIQ(PSR, VAL) ((PSR) = (((PSR) & ~(1 << 6)) | ((VAL & 1) << 6)))
#define _PSR_MODE(PSR)		((PSR) & 0x1f)
#define _SET_PSR_MODE(PSR, MODE) ((PSR) = (((PSR) & ~(0x1f)) | MODE)) 

/* Macro calls for setting and changing modes : Specifically, for finding location of banked registers */
#define USER_OFFSET	0
#define USER_SIZE		0

#define	FIQ_OFFSET         0
#define	FIQ_SIZE		8
#define FIQ_SPSR_POS		8
#define FIQ_CPSRMASK	0xFFFFFF20
#define FIQ_CPSRSET	((0x3 << 6) | FIQ_MODE)
#define FIQ_VECTOR		0x01C

#define IRQ_OFFSET		15
#define IRQ_SIZE		2
#define IRQ_SPSR_POS		17
#define IRQ_CPSRMASK      	0xFFFFFF60
#define IRQ_CPSRSET       	((0x1 << 7) | IRQ_MODE)
#define IRQ_VECTOR		0x018

#define SVC_OFFSET		9
#define SVC_SIZE		2
#define SVC_SPSR_POS		11
#define SVC_CPSRMASK      	0xFFFFFF60
#define SVC_CPSRSET       	((0x1 << 7) | SVC_MODE)
#define SVC_VECTOR		0x08

#define ABT_OFFSET		12
#define ABT_SIZE		2
#define ABT_SPSR_POS		14
#define ABT_CPSRMASK      	0xFFFFFF60
#define ABT_CPSRSET       	((0x1 << 7) | ABT_MODE)
#define ABT_VECTOR		0x10   /* FIXME: only DATA abort!! */

#define UND_OFFSET		18
#define UND_SIZE		2
#define UND_SPSR_POS		20
#define UND_CPSRMASK      	0xFFFFFF60
#define UND_CPSRSET       	((0x1 << 7) | UND_MODE)
#define UND_VECTOR		0x04

#define _INTERRUPT_VECTOR(MODE)   MODE##_VECTOR
#define _MODE_SET_CPSR(CPSR, MODE)   \
	((CPSR) = ((CPSR & MODE##_CPSRMASK) | MODE##_CPSRSET))

#ifndef MAX
#define MAX(x,y)   (((x)>(y)) ? (x) : (y))
#endif

#define _STORE_CPSR(CPSR, BANK_REGS, MODE)		\
memcpy( (word_t *)((int)(&(BANK_REGS)) + (MODE##_SPSR_POS * sizeof(word_t))),	\
	&CPSR,						\
	sizeof(word_t))

#define _LOAD_CPSR(CPSR, BANK_REGS, MODE)		\
memcpy( &CPSR,						\
        (word_t *)((int)(&(BANK_REGS)) + (MODE##_SPSR_POS * sizeof(word_t))),	\
	sizeof(word_t))
