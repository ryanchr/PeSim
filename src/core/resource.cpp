...
res_create_pool(char *name, struct res_desc *pool, int ndesc)
{
  ...
  for (index=0,i=0; i<ndesc; i++)
      ...
      for (j=0; j<pool[i].quantity; j++)
	{
	  ...
	  for (k=0; k<MAX_RES_CLASSES && inst_pool[index].x[k].rescclass; k++)
  ...
  for (i=0; i<ninsts; i++)
      ...
      for (j=0; j<MAX_RES_CLASSES; j++)
	{
	  ...
	  if (plate->rescclass)
	    {
	      assert(plate->rescclass < MAX_RES_CLASSES);
	      res->table[plate->rescclass][res->nents[plate->rescclass]++] = plate;
	    }
	  else
...
struct res_template *
res_get(struct res_pool *pool, int rescclass)
{
  int i;

  /* must be a valid class */
  assert(rescclass < MAX_RES_CLASSES);

  /* must be at least one resource in this class */
  assert(pool->table[rescclass][0]);

  for (i=0; i<MAX_INSTS_PER_CLASS; i++)
    {
      if (pool->table[rescclass][i])
	{
	  if (!pool->table[rescclass][i]->master->busy)
	    return pool->table[rescclass][i];
	}
      else
	break;
    }
