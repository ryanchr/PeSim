...
static int in_flow = FALSE, flow_index, nflow, Intpt = FALSE;
...
#define STATVAL(STAT)							\
	((STAT)->ss_sc == ss_sc_int							\
   ? (counter_t)*((STAT)->variant.for_int.var)			\
	: ((STAT)->ss_sc == ss_sc_uint						\
  ...
  for (i=0; i<pcstat_nelt; i++)
	...
	fatal("cannot locate any statistic named `%s'", pcstat_vars[i]);
      ...
      /* stat must be an integral type */
		if (stat->ss_sc != ss_sc_int && stat->ss_sc != ss_sc_uint && stat->ss_sc != sc_counter)
...
ruu_init(void)
{
	RUU = (struct RUU_station *)calloc(RUU_size, sizeof(struct RUU_station));
  if (!RUU)
...
lsq_init(void)
{
	LSQ = (struct RUU_station *)calloc(LSQ_size, sizeof(struct RUU_station));
  if (!LSQ)
    fatal("out of virtual memory");
...
/* ruu_clear processor microarchitecture state back to RUU(NULL)  */
static void
ruu_clear()			/* index of mis-pred branch */
{
       /*clear RUU and LSQ*/
       memset(RUU , NULL , RUU_size*sizeof(struct RUU_station));
       memset(LSQ , NULL , LSQ_size*sizeof(struct RUU_station));
	...
	BITMAP_CLEAR_MAP(use_spec_cv, CV_BMAP_SZ);
        /*revert the RUU state*/
     	RUU_num = 0;
    	RUU_head = RUU_tail = 0;
    	RUU_count = 0;
    	RUU_fcount = 0;
        /*revert the LSQ state*/
    	LSQ_num = 0;
    	LSQ_head = LSQ_tail = 0;
    	LSQ_count = 0;
    	LSQ_fcount = 0;
...
{
	fatal("spec_mem_access is not defined, use CoreArm::_SpecMemAccess instead\n");
}

fault_t CoreArm::_SpecMemAccess(	enum MemCmd cmd,		/* Read or Write access cmd */
  ...
  /* no, if it is a write, allocate a hash table entry to hold the data */
      ...
      if (!bucket_free_list)
	{
	  /* otherwise, call calloc() to get the needed storage */
			bucket_free_list = (struct spec_mem_ent *)calloc(1, sizeof(struct spec_mem_ent));
	  if (!bucket_free_list)
	    fatal("out of virtual memory");
	}
...
#define SET_GPR(N,EXPR)         ((void)(((N) == 15) ? setPC++ : 0),	\
				 ...
				 ? ((spec_regs_R[N] = (EXPR)),		\
				    ...
				    spec_regs_R[N])			\
	: regs.regs_R[N]= (EXPR))    /*, = regs.regs_R[N]  */
...
/* MODE to switch to, and PC to be saved  (w/ +4/+8 already included)  */		
#define SET_MODE(NEWMODE, PC, NPC)  					\
do{														\
  if (irq_debug)							\
...
#define __READ_SPECMEM(SRC, SRC_V, FAULT)				\
  (addr = (SRC),							\
   (spec_mode								\
	? ((FAULT) = _SpecMemAccess(CmdRead, addr, &SRC_V, sizeof(SRC_V)))\
	: ((FAULT) = _slow_mem->MemAccess(CmdRead, addr, &SRC_V, sizeof(SRC_V)))),	\
   SRC_V)
...
#define __WRITE_SPECMEM(SRC, DST, DST_V, FAULT)				\
  (DST_V = (SRC), addr = (DST),						\
   (spec_mode								\
	? ((FAULT) = _SpecMemAccess(CmdWrite, addr, &DST_V, sizeof(DST_V)))\
	: ((FAULT) = _slow_mem->MemAccess(CmdWrite, addr, &DST_V, sizeof(DST_V)))))
...
/*fetch_clear to clear the i-fetch of pipeline*/
static void
fetch_clear( )
{
	/* clear the IFETCH -> DISPATCH instruction queue */
    memset(fetch_data , NULL , ruu_ifq_size*sizeof(struct fetch_rec));
	fetch_num = 0;
	fetch_tail = fetch_head = 0;
	IFQ_count = 0;
	IFQ_fcount = 0;
...
bpred_clear()
{
  if (pred == NULL)
    return;
  pred->retstack.tos = NULL;
}

void CoreArm::_IrqHandler()
{
	printf("irq!!!!!!!!!!!!!!!!!!\n");
      /*clear pc of the current inst*/
	//regs.regs_PC = 0x0;
	//regs.regs_NPC = regs.regs_PC + sizeof(md_inst_t);
	regs.regs_R[MD_REG_PC] = 0x0;
      /*revert the RUU and LSQ state*/
      ruu_clear();
...
#define SYSCALL(INST)							\
  (/* only execute system calls in non-speculative mode */		\
   (spec_mode ? panic("speculative syscall") : (void) 0),		\
	sys_syscall_pesim(&regs, _slow_mem, INST, TRUE))
/* default register state accessor, used by DLite */
static char *					/* err str, NULL for no err */
...
/* WATTCH */
void CoreArm::_RuuDispatch(void)
{
  int i;
...
void CoreArm::_RuuFetch(void)
{
  ...
  for (i=0, branch_cnt=0;
	  ...
	  && !(fetch_regs_PC & (sizeof(md_inst_t)-1)))
	{
		  ...
		  _FetchInst(fetch_regs_PC, &inst);    /*FIXED*/
#if 0
...
{
	Uint32 inst_tmp;
	_slow_mem->MemAccess(CmdRead, regs.regs_PC, &inst_tmp, 4);
	md_print_insn(inst_tmp, regs.regs_PC, stderr);
	fprintf(stderr, "   (PC = 0x%08x  inst = 0x%08x)\n", regs.regs_PC, inst_tmp);	
...
void CoreArm::MainAction(void)
{
  ...
  /* fast forward simulator loop, performs functional simulation for
	...
	/*set the fast forward simulator loop , and it can also be set by user*/
	fastfwd_count=10;
  if (fastfwd_count > 0)
    {
      int icount;
...
#include "machine.def"
  ...
  /* set up timing simulation entry state */
       /*It has been commented out by ren.chen*/
      ...
      else
	{
      ...
      sim_cycle++;
			...
			printf("Core changed mode to IRQ\n");
                    /*store cpsr,PC and set interrupt PC*/
                   SET_PSR_MODE(IRQ, regs.regs_PC, regs.regs_NPC);
                    /*irq handler program*/
			_IrqHandler();    		
			 regs.regs_NPC = regs.regs_PC + sizeof(md_inst_t);
                    /*set the pred pc for bpred_update and ruu_fetch*/
                    fetch_regs_PC = regs.regs_PC ;
                   fetch_pred_PC =  regs.regs_PC;
			trapPC = TRUE;
                   Intpt = TRUE;
