...
void
sys_syscall_pesim(struct regs_t *regs,	/* registers to access */
	    Bus_slow_mem *mem_slow,		/* memory space to access */
	    ...
	    int traceable)		/* traceable system call? */
{
  ...
  /* Figure out the system call number from the swi instruction
	...
	fprintf(fout, "sys_syscall_pesim inst = 0x%x\n", inst);
  ...
  switch (syscode)
    ...
    case ARM_SYS_exit:
	  ...
	  longjmp(g_core_exit, (regs->regs_R[MD_REG_R0] & 0xff) + 1);
	...
	mem_slow->mem_rbcopy((Uint32)regs->regs_R[MD_REG_R1],(Sint32*)buf,(Sint32)regs->regs_R[MD_REG_R2]);
	...
	//mem_slow->mem_rbcopy(/*buf*/(Uint32)regs->regs_R[MD_REG_R1], (Sint32*)buf,
		//(Sint32)regs->regs_R[MD_REG_R2]);
	mem_slow->MemBcopy(CmdRead, regs->regs_R[MD_REG_R1], buf, regs->regs_R[MD_REG_R2]);
	...
	mem_slow->mem_wbcopy((/*buf*/(Uint32)regs->regs_R[MD_REG_R0], (Sint32*)buf,
		(Sint32)regs->regs_R[MD_REG_R1]);
      }
	...
	mem_slow->mem_rbcopy(regs->regs_R[MD_REG_A1], (Sint32*)&a_sock,
		(Sint32)regs->regs_R[MD_REG_A2]);
	...
	mem_slow->mem_rbcopy(regs->regs_R[MD_REG_A1], (Sint32*)buf,
		(Sint32)regs->regs_R[MD_REG_A2]);

	if (regs->regs_R[MD_REG_A5] > 0)
		mem_slow->mem_rbcopy(regs->regs_R[MD_REG_A4], (Sint32*)&d_sock,
		(Sint32)regs->regs_R[MD_REG_A5]);
	...
	//mem_bcopy(mem_fn, mem, Write, /* serv_addr */regs->regs_R[MD_REG_A1],
		  //buf, /* addrlen */(int)regs->regs_R[MD_REG_A2]);
	mem_slow->mem_wbcopy(regs->regs_R[MD_REG_A1], (Sint32*)buf,
		(Sint32)regs->regs_R[MD_REG_A2]);
	
	/* maybe copy back whole size of sockaddr */
	if (regs->regs_R[MD_REG_A5] > 0)
		mem_slow->mem_wbcopy(regs->regs_R[MD_REG_A4], (Sint32*)&d_sock,
		(Sint32)regs->regs_R[MD_REG_A5]);
	...
	//mem_bcopy(mem_fn, mem, Read, /* serv_addr */regs->regs_R[MD_REG_A5],
		  //&addr_len, sizeof(int));
	mem_slow->mem_rbcopy(regs->regs_R[MD_REG_A5], (Sint32*)&addr_len,
		(Sint32)sizeof(int));
	...
	//mem_bcopy(mem_fn, mem, Read, regs->regs_R[MD_REG_A4],
		  //a_sock, addr_len);
	mem_slow->mem_rbcopy(regs->regs_R[MD_REG_A4], (Sint32*)a_sock,
		(Sint32)addr_len);
	...
	mem_slow->mem_wbcopy(regs->regs_R[MD_REG_A1], (Sint32*)buf,
		(Sint32)regs->regs_R[MD_REG_V0]);

	//mem_bcopy(mem_fn, mem, Write, /* serv_addr */regs->regs_R[MD_REG_A5],
		  //&addr_len, sizeof(int));
	mem_slow->mem_wbcopy(regs->regs_R[MD_REG_A5], (Sint32*)&addr_len,
		(Sint32)sizeof(int));

	//mem_bcopy(mem_fn, mem, Write, regs->regs_R[MD_REG_A4],
		  //a_sock, addr_len);
	mem_slow->mem_wbcopy(regs->regs_R[MD_REG_A4], (Sint32*)a_sock,
		(Sint32)addr_len);
	...
	mem_slow->mem_rstrcpy((Uint32)regs->regs_R[MD_REG_R0], (char *)buf);
	...
	mem_slow->mem_wbcopy((Uint32)regs->regs_R[MD_REG_R1],
		(Sint32*)&linux_sbuf, (Sint32)sizeof(struct linux_statbuf));
      }
      break;
	...
	mem_slow->mem_wbcopy((Uint32)regs->regs_R[MD_REG_R0],
		  (Sint32*)&tms, sizeof(struct linux_tms));
	...
	case OSF_TIOCGETP: /* <Out,TIOCGETP,6> */
	  {
		...
		mem_slow->mem_wbcopy((Uint32)regs->regs_R[MD_REG_R2], (Sint32*)&buf,
		      sizeof(struct osf_sgttyb));
	...
	case OSF_FIONREAD:
	  {
	    ...
	    //mem_bcopy(mem_fn, mem, Write,
		     ...
		     // &nread, sizeof(struct osf_sgttyb));
		mem_slow->mem_wbcopy((Uint32)regs->regs_R[MD_REG_R2], (Sint32*)&nread,
		      sizeof(struct osf_sgttyb));
	...
	case /*FIXME*/FIONBIO:
	  {
	    ...
	    if (regs->regs_R[MD_REG_R2])
			mem_slow->mem_rbcopy((Uint32)regs->regs_R[MD_REG_R2], (Sint32*)&arg,
		      sizeof(arg));
	    ...
	    if (regs->regs_R[MD_REG_R2])
			mem_slow->mem_wbcopy((Uint32)regs->regs_R[MD_REG_R2], (Sint32*)&arg,
		      sizeof(arg));
	      //mem_bcopy(mem_fn, mem, Write,
		      ///* arg */regs->regs_R[MD_REG_R2],
		      //&arg, sizeof(arg));
...
/* #endif */
	...
	//mem_bcopy(mem_fn, mem, Write, /*sbuf*/regs->regs_R[MD_REG_R1],
		  //&linux_sbuf, sizeof(struct linux_statbuf));
	mem_slow->mem_wbcopy((Uint32)regs->regs_R[MD_REG_R1],
		  (Sint32*)&linux_sbuf, sizeof(struct linux_statbuf));
      }
      break;
	  ...
	  if (table_index != 0)
		...
		(rusage_info.ru_stime.tv_usec * sysinfo.si_hz) / 1000000UL;
		 ...
		 are ignored */
		  ...
		  mem_slow->mem_wbcopy((Uint32)buf_addr,
			(Sint32*)&sysinfo, sizeof(struct osf_tbl_sysinfo));
...
/*----------------------------------------------------------------------------------*/
	...
	if (/*timeval*/regs->regs_R[MD_REG_R0] != 0)
	  {
		...
		mem_slow->mem_rbcopy((Uint32)regs->regs_R[MD_REG_R0],
		      (Sint32*)&osf_tv, sizeof(struct osf_timeval));
	...
	if (/*timezone*/regs->regs_R[MD_REG_R1] != 0)
	  {
		...
		mem_slow->mem_rbcopy((Uint32)regs->regs_R[MD_REG_R1],
		      (Sint32*)&osf_tz, sizeof(struct osf_timezone));
	...
	mem_slow->mem_wbcopy((Uint32)regs->regs_R[MD_REG_A1],
		  (Sint32*)&rusage, sizeof(struct osf_rusage));
      }
...
/*-------------------------------Is there a utimes in arm-linux??----------------------*/
	...
	mem_slow->mem_rstrcpy((Uint32)regs->regs_R[MD_REG_A0], (char *)buf);
...
#error No utimes() implementation!
	...
	else
	  {
		...
		mem_slow->mem_rbcopy((Uint32)regs->regs_R[MD_REG_A1],
		  (Sint32*)osf_tval, 2*sizeof(struct osf_timeval));
...
/*----------------------------------------------------------------------------------*/
	...
	mem_slow->mem_rbcopy((Uint32)regs->regs_R[MD_REG_R1],
		  (Sint32*)&osf_rl, sizeof(struct osf_rlimit));
	...
	//mem_bcopy(mem_fn, mem, Read, /* sc */regs->regs_R[MD_REG_R0],
		  //&sc, sizeof(struct osf_sigcontext));
	mem_slow->mem_rbcopy((Uint32)regs->regs_R[MD_REG_R0],
		  (Sint32*)&sc, sizeof(struct osf_sigcontext));
	...
	if (regs->regs_R[MD_REG_V0] != (qword_t)-1)
	  {
		...
		for (i=0, cnt=0, osf_cnt=0, p=(struct dirent *)buf;
		     ...
		     i++, cnt += p->d_reclen, p=(struct dirent *)(buf+cnt))
		  {
			...
			mem_slow->mem_wbcopy((Uint32)(osf_buf + osf_cnt),
				(Sint32*)&osf_dirent, OSF_DIRENT_SZ(p->d_name));
		...
		if (osf_pbase != 0)
		  {
			...
			mem_slow->mem_wbcopy((Uint32)osf_pbase,
				(Sint32*)&osf_base, sizeof(osf_base));
		  }
...
/*-----------------------------------------------------------------------------------------*/
	...
	//mem_bcopy(mem_fn, mem, Write, /*sbuf*/regs->regs_R[MD_REG_R1],
	//&osf_sbuf, sizeof(struct osf_statfs)); 
	mem_slow->mem_wbcopy((Uint32)regs->regs_R[MD_REG_R1],
				(Sint32*)&osf_sbuf, sizeof(struct osf_statfs));
    ...
    case ARM_SYS_socket:
        ...
        //mem_bcopy(mem_fn, mem, Read, regs->regs_R[MD_REG_R1], &__domain,
		  ///*nbytes*/sizeof(int));
		mem_slow->mem_rbcopy((Uint32)regs->regs_R[MD_REG_R1],
				(Sint32*)&__domain, sizeof(int));
        //mem_bcopy(mem_fn, mem, Read, (regs->regs_R[MD_REG_R1]+sizeof(int)), &__type,
		  ///*nbytes*/sizeof(int));
		mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+sizeof(int)),
				(Sint32*)&__type, sizeof(int));
        //mem_bcopy(mem_fn, mem, Read, (regs->regs_R[MD_REG_R1]+2*sizeof(int)), 
			//&__protocol,/*nbytes*/sizeof(int));
		mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+2*sizeof(int)),
				(Sint32*)&__protocol, sizeof(int));
	...
	//mem_bcopy(mem_fn, mem, Read, /* serv_addr */regs->regs_R[MD_REG_R1],
		  //&osf_sa, /* addrlen */(int)regs->regs_R[MD_REG_R1]);
	mem_slow->mem_rbcopy((Uint32)regs->regs_R[MD_REG_R1],
				(Sint32*)&osf_sa, (int)regs->regs_R[MD_REG_R1]);
#if 0
	int i;
...
/* Should olduname and oldolduname be supported ?? ctw*/
	...
	for (i=0; i < /* iovcnt */regs->regs_R[MD_REG_R2]; i++)
	  {
		...
		mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1] + i*sizeof(struct osf_iovec)),
				(Sint32*)&osf_iov, sizeof(struct osf_iovec));
		...
		mem_slow->mem_rbcopy((Uint32)(MD_SWAPQ(osf_iov.iov_base)),
				(Sint32*)&buf, (Sint32)MD_SWAPW(osf_iov.iov_len));
		iov[i].iov_base = buf;
	      }
	    else
	      iov[i].iov_base = NULL;
	...
	/* allocate host side I/O vectors */
	osf_iov =
	  (struct osf_iovec *)calloc(/* iovcnt */regs->regs_R[MD_REG_R2],
		 sizeof(struct osf_iovec));
	if (!osf_iov)
	  fatal("out of virtual memory in SYS_readv");

	iov =
	  (struct iovec*)calloc(/* iovcnt */regs->regs_R[MD_REG_R2], sizeof(struct iovec));
	if (!iov)
	  fatal("out of virtual memory in SYS_readv");
	...
	for (i=0; i < /* iovcnt */regs->regs_R[MD_REG_R2]; i++)
	  {
		...
		mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1] + i*sizeof(struct osf_iovec)),
				(Sint32*)&osf_iov[i], sizeof(struct osf_iovec));
	...
	for (i=0; i < /* iovcnt */regs->regs_R[MD_REG_R2]; i++)
	  {
		...
		mem_slow->mem_wbcopy((Uint32)(MD_SWAPQ(osf_iov[i].iov_base)),
				(Sint32*)iov[i].iov_base, (int)MD_SWAPW(osf_iov[i].iov_len));
	      }
	  }
...
/*------------Subcall of socketcall-------------------------------*/
	...
	if (/* optval */regs->regs_R[MD_REG_A3] != 0
	    && /* optlen */regs->regs_R[MD_REG_A4] != 0)
	  {
		...
		mem_slow->mem_rbcopy((Uint32)regs->regs_R[MD_REG_A3],
				(Sint32*)buf, (int)regs->regs_R[MD_REG_A4]);
	  }
	else
	  buf = NULL;
...
/* subcall of socketcall in arm linux */
	...
	mem_slow->mem_rbcopy((Uint32)regs->regs_R[MD_REG_A2],
				(Sint32*)&osf_addrlen, sizeof(osf_addrlen));
	...
	//mem_bcopy(mem_fn, mem, Write,
		  ///* paddrlen */regs->regs_R[MD_REG_A2],
		  //&osf_addrlen, sizeof(osf_addrlen));
	mem_slow->mem_wbcopy((Uint32)regs->regs_R[MD_REG_A2],
				(Sint32*)&osf_addrlen, sizeof(osf_addrlen));
...
/*-----------------------------------------------------------------------------------*/
	...
	if (/* writefds */regs->regs_R[MD_REG_R2] != 0)
	  {
		...
		mem_slow->mem_rbcopy((Uint32)regs->regs_R[MD_REG_R2],
				(Sint32*)&readfd, sizeof(fd_set));
	    writefdp = &writefd;
	  }
	else
	...
	if (/* exceptfds */regs->regs_R[MD_REG_R3] != 0)
	  {
		...
		mem_slow->mem_rbcopy((Uint32)regs->regs_R[MD_REG_R3],
				(Sint32*)&exceptfd, sizeof(fd_set));
	    exceptfdp = &exceptfd;
	  }
	else
	  exceptfdp = NULL;
	...
	if (/* timeout */regs->regs_R[MD_REG_R4] != 0)
	  {
		...
		mem_slow->mem_rbcopy((Uint32)regs->regs_R[MD_REG_R4],
				(Sint32*)&timeout, sizeof(struct timeval));
	    timeoutp = &timeout;
	  }
	else
	  timeoutp = NULL;
	...
	if (/* writefds */regs->regs_R[MD_REG_R2] != 0)
		mem_slow->mem_wbcopy((Uint32)regs->regs_R[MD_REG_R2],
				(Sint32*)&writefd, sizeof(fd_set));
	  //mem_bcopy(mem_fn, mem, Write,
		    ///* writefds */regs->regs_R[MD_REG_R2],
		    //&writefd, sizeof(fd_set));
	...
	if (/* exceptfds */regs->regs_R[MD_REG_R3] != 0)
		mem_slow->mem_wbcopy((Uint32)regs->regs_R[MD_REG_R3],
				(Sint32*)&exceptfd, sizeof(fd_set));
	  //mem_bcopy(mem_fn, mem, Write,
		   // /* exceptfds */regs->regs_R[MD_REG_R3],
		    //&exceptfd, sizeof(fd_set));
	...
	if (/* timeout */regs->regs_R[MD_REG_R4] != 0)
		mem_slow->mem_wbcopy((Uint32)regs->regs_R[MD_REG_R4],
				(Sint32*)&timeout, sizeof(struct timeval));
	  //mem_bcopy(mem_fn, mem, Write,
		    ///* timeout */regs->regs_R[MD_REG_R4],
		    //&timeout, sizeof(struct timeval));
...
/* part of socketcall in arm linux */
	...
	/* allocate host side I/O vectors */
	fds = (struct pollfd*)calloc(/* nfds */regs->regs_R[MD_REG_R1], sizeof(struct pollfd));
	if (!fds)
	...
	for (i=0; i < /* nfds */regs->regs_R[MD_REG_R1]; i++)
	  {
		...
		mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R0] + i*sizeof(struct pollfd)),
				(Sint32*)&fds[i], sizeof(struct pollfd));
	  }
	...
	for (i=0; i < /* nfds */regs->regs_R[MD_REG_R1]; i++)
	  {
	    ...
	    //mem_bcopy(mem_fn, mem, Write,
		      ...
		      // + i*sizeof(struct pollfd)),
		      //&fds[i], sizeof(struct pollfd));
		mem_slow->mem_wbcopy((Uint32)(regs->regs_R[MD_REG_R0] + i*sizeof(struct pollfd)),
				(Sint32*)&fds[i], sizeof(struct pollfd));
	  }
...
/* part of socketcall in arm linux */
	...
	mem_slow->mem_wbcopy((Uint32)regs->regs_R[MD_REG_A0],
			(Sint32*)&buf, (int)regs->regs_R[MD_REG_A1]);
      }
      break;
#endif
...
/* The entry into the sockets function calls!!! */
	  ...
	  /* got an error, return details */
          ...
          case ARM_SYS_BIND:
          {
             ...
             //mem_bcopy(mem_fn, mem, Read,regs->regs_R[MD_REG_R0], 
                    //&__sockfd,sizeof(int));
			 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R0]),
				(Sint32*)&__sockfd, sizeof(int));
	     //mem_bcopy(mem_fn, mem, Read,(regs->regs_R[MD_REG_R0]+sizeof(int)),
		    //&__sim_a_sock, sizeof(int));
		 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R0]+sizeof(int)),
				(Sint32*)&__sim_a_sock, sizeof(int));
	     //mem_bcopy(mem_fn, mem, Read,
                    //(regs->regs_R[MD_REG_R0]+2*sizeof(int)),
		    //&__addrlen, sizeof(int));
		 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R0]+2*sizeof(int)),
				(Sint32*)&__addrlen, sizeof(int));

             /* copy the sockadd to real memory */
             //mem_bcopy(mem_fn, mem, Read,__sim_a_sock,
		    //&a_sock, sizeof(struct sockaddr));
			 mem_slow->mem_rbcopy((Uint32)(__sim_a_sock),
				(Sint32*)&a_sock, sizeof(struct sockaddr));
	...
	mem_slow->mem_rbcopy((Uint32)(__sim_addr),
				(Sint32*)&osf_sa, sizeof(struct osf_sockaddr));
...
#endif
	/* result */regs->regs_R[MD_REG_R0] =
	  connect(/* sockfd */__sockfd, (const sockaddr*)&osf_sa, __addrlen);
          ...
          case ARM_SYS_ACCEPT:
	  {
	    ...
	    //mem_bcopy(mem_fn, mem, Read,(regs->regs_R[MD_REG_R0]+2*sizeof(int)),
		 ...
		 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R0]),
				(Sint32*)&__s, sizeof(int));
	  	 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R0]+sizeof(int)),
				(Sint32*)&__name, sizeof(int));
		 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R0]+2*sizeof(int)),
				(Sint32*)&__namelen, sizeof(int));

	    /* get simulator memory parameters to host memory */
	    //mem_bcopy(mem_fn, mem, Read,
		      ///* paddrlen */__namelen,
		  //&osf_addrlen, sizeof(osf_addrlen));
		mem_slow->mem_rbcopy((Uint32)(__namelen),
				(Sint32*)&osf_addrlen, sizeof(osf_addrlen));
	    addrlen = (int)osf_addrlen;
	    if (addrlen != 0)
	    {
	    ...
	    //mem_bcopy(mem_fn, mem, Write,
		  ///* paddrlen */__namelen,
		  //&osf_addrlen, sizeof(osf_addrlen));
		mem_slow->mem_wbcopy((Uint32)(__namelen),
				(Sint32*)&osf_addrlen, sizeof(osf_addrlen));
	  ...
	  /* copy results to simulator memory */
	  if (addrlen != 0)
	  	mem_slow->mem_wbcopy((Uint32)__name,
				(Sint32*)buf, addrlen);
	    //mem_bcopy(mem_fn, mem, Write,
		    ///* addr */__name,
		   // buf, addrlen);
          ...
          case ARM_SYS_RECV:
	  {
	    ...
	    /*mem_bcopy(mem_fn, mem, Read,
		 ...
		 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]),
				(Sint32*)&__s, sizeof(int));
	  	 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+sizeof(int)),
				(Sint32*)&__msg, sizeof(int));
		 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+2*sizeof(int)),
				(Sint32*)&__len, sizeof(int));
		 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+3*sizeof(int)),
				(Sint32*)&__flags, sizeof(int));
	  	 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+4*sizeof(int)),
				(Sint32*)&__to, sizeof(int));
		 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+5*sizeof(int)),
				(Sint32*)&__tolen, sizeof(int));
	     ...
	     /* copy the message from simualted memory to host memory */
		  ...
		  //buf, /* addrlen */__len);
		 mem_slow->mem_rbcopy((Uint32)__msg,
				(Sint32*)buf, __len);

	     if (__tolen > 0)
		 	mem_slow->mem_rbcopy((Uint32)__to,
				(Sint32*)&d_sock, __tolen);
	     ...
	     //mem_bcopy(mem_fn, mem, Write, /* serv_addr*/__msg,
		  //buf, /* addrlen */__len);
		 mem_slow->mem_wbcopy((Uint32)__msg,
				(Sint32*)buf, __len);

	   /* maybe copy back whole size of sockaddr */
	   if (__tolen > 0)
	   	mem_slow->mem_wbcopy((Uint32)__to,
				(Sint32*)&d_sock, __tolen);
	   ...
	   /* Not sure what to do with the error conditions yet */
	    ...
	    /*mem_bcopy(mem_fn, mem, Read,
		 ...
		 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]),
				(Sint32*)&__s, sizeof(int));
	  	 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+sizeof(int)),
				(Sint32*)&__buf, sizeof(int));
		 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+2*sizeof(int)),
				(Sint32*)&__len, sizeof(int));
		 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+3*sizeof(int)),
				(Sint32*)&__flags, sizeof(int));
	  	 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+4*sizeof(int)),
				(Sint32*)&__from, sizeof(int));
		 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+5*sizeof(int)),
				(Sint32*)&__fromlen, sizeof(int));

	    buf = (char *) malloc((__len));

	    //mem_bcopy(mem_fn, mem, Read, /* serv_addr */__buf,
		  //buf, /* addrlen */__len);
		mem_slow->mem_rbcopy((Uint32)__buf,
				(Sint32*)buf, __len);

	    //mem_bcopy(mem_fn, mem, Read, /* serv_addr */__fromlen,
		  //&addr_len, sizeof(int));
		mem_slow->mem_rbcopy((Uint32)__fromlen,
				(Sint32*)&addr_len, sizeof(int));
	    ...
	    //mem_bcopy(mem_fn, mem, Read, __from,
		  //a_sock, addr_len);
		mem_slow->mem_rbcopy((Uint32)__from,
				(Sint32*)a_sock, addr_len);

            /* make the actual system call */
	    regs->regs_R[MD_REG_R0] =
	    recvfrom(__s, buf,__len,__flags, a_sock,(socklen_t*)&addr_len);

	    //mem_bcopy(mem_fn, mem, Write, __buf,
		  //buf, (int) regs->regs_R[MD_REG_R0]);
		mem_slow->mem_wbcopy((Uint32)__buf,
				(Sint32*)buf, (int) regs->regs_R[MD_REG_R0]);

	    //mem_bcopy(mem_fn, mem, Write, /* serv_addr */__fromlen,
		  //&addr_len, sizeof(int));
		mem_slow->mem_wbcopy((Uint32)__fromlen,
				(Sint32*)&addr_len, sizeof(int));

	    //mem_bcopy(mem_fn, mem, Write, __from,
		  //a_sock, addr_len);
		mem_slow->mem_wbcopy((Uint32)__from,
				(Sint32*)a_sock, addr_len);
          ...
          case ARM_SYS_SHUTDOWN:
	  {
	    ...
	    //mem_bcopy(mem_fn, mem, Read,
		  ...
		  //&__arg1, sizeof(int));
		mem_slow->mem_wbcopy((Uint32)regs->regs_R[MD_REG_R1],
				(Sint32*)&__arg1, sizeof(int));

            //mem_bcopy(mem_fn, mem, Read,
		  //(regs->regs_R[MD_REG_R1]+sizeof(int)),
		  //&__arg2, sizeof(int));
			mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+sizeof(int)),
				(Sint32*)&__arg2, sizeof(int));
            /* shuts down socket send and receive operations */
	    ...
	    /*mem_bcopy(mem_fn, mem, Read,
		 ...
		 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]),
				(Sint32*)&__s, sizeof(int));
	  	 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+sizeof(int)),
				(Sint32*)&__level, sizeof(int));
		 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+2*sizeof(int)),
				(Sint32*)&__optname, sizeof(int));
		 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+3*sizeof(int)),
				(Sint32*)&__optval, sizeof(int));
	  	 mem_slow->mem_rbcopy((Uint32)(regs->regs_R[MD_REG_R1]+4*sizeof(int)),
				(Sint32*)&__optlen, sizeof(int));
	    ...
	    if (/* optval */__optval != 0
	       && /* optlen */__optlen != 0)
	    {
	      buf = (char*)calloc(1, /* optlen */(size_t)__optlen);
	      if (!buf)
	        fatal("cannot allocate memory in OSF_SYS_setsockopt");
	      ...
	      //mem_bcopy(mem_fn, mem, Read,
		      ///* optval */__optval,
		      //buf, /* optlen */__optlen);
		  mem_slow->mem_rbcopy((Uint32)__optval,
				(Sint32*)buf, __optlen);
	    }
	    else
	      buf = NULL;
